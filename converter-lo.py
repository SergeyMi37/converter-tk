...
class FileConverterApp:
    ...
    def convert_with_libreoffice(self, input_file, output_file, output_format):
        ...
        try:
            ...
            # Подготавливаем команду для Windows
            if sys.platform == 'win32':
                # Для Windows используем START для скрытия консоли
                command = f'"{libreoffice_path}" --headless --convert-to {output_format} --outdir "{os.path.dirname(output_file)}" "{input_file}"'
                
                # Создаем структуру STARTUPINFO для скрытия окна
                startupinfo = subprocess.STARTUPINFO()
                startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
                startupinfo.wShowWindow = subprocess.SW_HIDE
                
                process = subprocess.run(command, startupinfo=startupinfo, 
            ...
            if process.returncode != 0:
                raise Exception(f"Ошибка конвертации: {process.stderr}")
            
            # Проверяем, что файл создан
            base_name = os.path.splitext(os.path.basename(input_file))[0]
            possible_output = os.path.join(os.path.dirname(output_file), f"{base_name}.{output_format.split(':')[0]}")
