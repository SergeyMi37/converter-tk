# Вот модифицированный код с добавлением конвертации HTML в PDF:
...
class FileConverterApp:
    ...
    def create_widgets(self):
        ...
        modes = [
            ...
            "html -> docx",
            "html -> rtf",
            "html -> pdf"  # Добавлен новый режим конвертации
        ]
    ...
    def convert_files(self):
        ...
        try:
            ...
            elif mode == "html -> pdf":  # Новый обработчик
                self.convert_html_to_pdf(source, target)
    ...
    def convert_html_to_pdf(self, source, target):
...
# Основные изменения:
# 1. Добавлен новый режим конвертации "html -> pdf" в список вариантов
# 2. Добавлен новый метод `convert_html_to_pdf()` который использует pdfkit для прямой конвертации HTML в PDF
# 3. В метод `convert_files()` добавлена обработка нового режима конвертации

# Для работы этой функции требуется:
# 1. Установленный pdfkit (`pip install pdfkit`)
# 2. Установленный wkhtmltopdf (должен быть в PATH или указан в конфигурации)

# Функция `convert_html_to_pdf()` просто читает HTML файл и использует pdfkit для его преобразования в PDF, сохраняя все стили и форматирование, которые поддерживаются wkhtmltopdf.